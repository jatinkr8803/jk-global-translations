...
form?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Fallback: if backend isn't running, show a friendly message
    if (err.message.includes('fetch') || err.name === 'TypeError') {
      showStatus('success', '✓ Thank you! We\'ll be in touch at ' + email + ' within 2 hours. (Demo mode — connect backend for full functionality)');
      form.reset();
    } else {
      showStatus('error', 'Something went wrong. Please try again or email us directly.');
    }
  } finally {
